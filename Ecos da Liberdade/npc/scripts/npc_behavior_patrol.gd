...
func _process( _delta: float ) -> void:
	...
	npc.direction = global_position.direction_to( target.target_position )
	npc.velocity = npc.direction * walk_speed
	
	if global_position.distance_to(target.target_position) < (walk_speed * _delta):
		idle_phase()
...
func gather_patrol_locations( _n : Node = null ) -> void:
	...
	patrol_locations.sort_custom( sort_by_patrol_id )
	...
	for i in patrol_locations.size():
		...
		var _next = patrol_locations[(i + 1) % patrol_locations.size()]
...
func sort_by_patrol_id ( instA: PatrolLocation , instB: PatrolLocation ) -> bool: 
	return instA.patrol_id < instB.patrol_id
